...
export class PermitV2 implements PermitV2Interface {
  ...
  static async create(options: PermitV2Options) {
    ...
    if (!success) {
      throw new Error("PermitV2 :: create :: Parsing PermitV2Options failed " + JSON.stringify(error, null, 2));
    }
   ...
   */
  sign = async (chainId: string | undefined, signer: AbstractSigner | undefined) => {
    if (chainId == null)
